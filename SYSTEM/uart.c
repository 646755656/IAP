...
void UART4_PutChar(unsigned char ch)
{
	...
	while(USART_GetFlagStatus(UART4, USART_FLAG_TXE) == RESET);//等待数据发送完毕
}

//*******************************************************************************
// 函数名称    : UART4_PutString()
// 功能描述    : 串口4发送数据函数.
// 输入        : u8 *buf 要发送的数据,len发送数据的长度
// 输出        : None
// 返回        : None
...
void UART5_PutChar(unsigned char ch)
{
	...
	while(USART_GetFlagStatus(UART5, USART_FLAG_TXE) == RESET);//等待数据发送完毕
}

//*******************************************************************************
// 函数名称    : UART5_PutString()
// 功能描述    : 串口5发送数据函数.
// 输入        : u8 *buf 要发送的数据,len发送数据的长度
// 输出        : None
// 返回        : None
